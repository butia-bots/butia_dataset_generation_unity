...
namespace Unity.Simulation
{
    ...
    {
        internal const string kVersionString       = "v0.0.10-preview.25";
        ...
        {
            Action action;
            while (_endOfFrameActionQueue.TryDequeue(out action))
                action?.Invoke();
        }
        ...
        public string GetDirectoryFor(string type = "", string userPath = "")
        {
            ...
            if (string.IsNullOrEmpty(userPath))
            {
                ...
                if (type == DataCapturePaths.Chunks)
                {
                    basePath = Path.Combine(Configuration.Instance.GetStoragePathOther(), type);
                }
            }
            else
            {
